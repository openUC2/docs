...
3. If the network service is supposed to be accessed via port 80, 443, or 8000, check whether other network services (e.g. ImSwitch, the openUC2 offline documentation, and the system file manager) are available over those ports by [opening their browser apps](../day-1/access.md#browser-apps).

4. If the network service binds directly to a port on the machine, [check the network port bindings](#how-to-check-network-port-bindings) to determine whether the network service is bound to its expected port.
...
5. Check whether the network service is accessible over a local connection from within the machine (i.e. as `localhost`).

6. [Check the firewall settings](#how-to-check-firewall-settings) on the machine to determine whether the firewall is configured to make the network service (and any network ports which it binds to) accessible over external network connections.
...
1. [Open the browser app](../day-1/access.md#browser-apps) for Dozzle.
...
3. Open the "Networking" page using Cockpit's navigation sidebar.
4. In the "Firewall" page, click on the "Edit rules and zones" button.
5. If the port needs to be accessible over [direct connections to the machine](../day-1/connectivity.md#directly), add a firewalld service for the port in the "NetworkManager Shared" zone:
...
6. If the port needs to be accessible over indirect connections to the machine [via a LAN](../day-1/connectivity.md#via-a-lan), add a firewall service for the port in the "Public" zone:
...
1. [Enter the machine's terminal](../day-1/access.md#the-machines-terminal).
2. If the port needs to be accessible over [direct connections to the machine](../day-1/connectivity.md#directly), add a rule for the port in the "nm-shared" zone:
   1. Run the following command:
      ...
      sudo tee -a <<<'  <port port="8080" protocol="tcp"/>' \
         /etc/firewalld/zones.d/nm-shared/80-custom-ports.xml
      ```
3. If the port needs to be accessible over indirect connections to the machine [via a LAN](../day-1/connectivity.md#via-a-lan), add a rule for the port in the "public" zone:
   1. Run the following command:
      ```bash
      sudo tee -a <<<'  <port port="{port}" protocol="{protocol}"/>' \
...
- Your machine has a secure [password for the `pi` user](../day-1/security.md#how-to-change-the-pi-users-password), because it can be used for remotely [accessing the machine's terminal](../day-1/access.md#the-machines-terminal).
