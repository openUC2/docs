...
Configuration variables in [`/etc/firewalld/firewalld.conf`](https://firewalld.org/documentation/configuration/firewalld-conf.html) are set with drop-in files in `/etc/firewalld/firewalld.conf.d`:

| Setting                          | ...in drop-in file             | ...from                                             | ...is set by default? |
| -------------------------------- | ------------------------------ | --------------------------------------------------- | --------------------- |
| `DefaultZone`:<br />`public`     | `20-default-zone.conf`         | `networking/firewalld`                              | yes                   |
| `StrictForwardPorts`:<br />`yes` | `20-strict-forward-ports.conf` | `networking/firewalld`:<br /> `govern-docker-ports` | no                    |

TODO: describe systemd services and `/run` file paths

## Services

firewalld is configured with the following [services](https://firewalld.org/documentation/service/):
...
| `http`                   | firewalld                                                                               | yes                   |
| `https`                  | firewalld                                                                               | yes                   |
| `http-ingress-untrusted` | `infra/caddy-ingress-untrusted`:<br /> `firewall-allow-direct`, `firewall-allow-public` | yes                   |
| `esp32-ota-firmware`     | `imswitch`:<br /> `firewall-allow-direct`, `firewall-allow-public`                      | yes                   |
| `imswitch-dev`           | `imswitch`:<br /> `firewall-allow-direct`, `firewall-allow-public`                      | yes                   |
...
| `http`                   | 80               |           |
| `https`                  | 443              |           |
| `http-ingress-untrusted` | 8000             |           |
| `esp32-ota-firmware`     | 3333             | 3232      |
| `imswitch-dev`           | 8001, 8888       |           |

## Zones

firewalld is configured with the following [zones](https://firewalld.org/documentation/zone/):
...
The `nm-shared` zone `/etc/firewalld/zones/nm-shared.xml` is defined by drop-in files in `/etc/firewalld/zones.d/nm-shared/`:

| Setting                                 | ...in drop-in file                              | ...from                                                                | ...is set by default? |
| --------------------------------------- | ----------------------------------------------- | ---------------------------------------------------------------------- | --------------------- |
| interface<br /> `tailscale0`            | `20-interface-tailscale0.xml`                   | `networking/tailscale`:<br /> `firewall-zone-nm-shared`                | yes                   |
...
| service<br /> `http-ingress-untrusted`  | `60-service-http-ingress-untrusted.xml`         | `infra/caddy-ingress-untrusted`:<br /> `firewall-allow-direct`         | yes                   |
...
The `public` zone `/etc/firewalld/zones/public.xml` is defined by drop-in files in `/etc/firewalld/zones.d/public/`:

| Setting                                 | ...in drop-in file                              | ...from                                                                | ...is set by default? |
| --------------------------------------- | ----------------------------------------------- | ---------------------------------------------------------------------- | --------------------- |
| interface<br /> `tailscale0`            | `20-interface-tailscale0.xml`                   | `networking/tailscale`:<br /> `firewall-zone-public`                   | no                    |
...
| service<br /> `http`                    | `60-service-http.xml`                           | `infra/caddy-ingress`:<br /> `firewall-allow-public`                   | no                    |
| service<br /> `https`                   | `60-service-https.xml`                          | `infra/caddy-ingress`:<br /> `firewall-allow-public`                   | no                    |
| service<br /> `http-ingress-untrusted`  | `60-service-http-ingress-untrusted.xml`         | `infra/caddy-ingress-untrusted`:<br /> `firewall-allow-public`         | yes                   |
| forward `:80` to<br /> `127.0.0.1:8000` | `70-forward-http-to-http-ingress-untrusted.xml` | `infra/caddy-ingress-untrusted`:<br /> `firewall-forward-http-public`  | yes                   |
