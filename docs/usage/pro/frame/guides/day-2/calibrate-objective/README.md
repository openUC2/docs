...
Objective calibration is recommended after you have done any mechanical work on the objective holder, e.g. removing it or exchanging an objective or if you notice, that when switching objectives the structure of interest will not remain centered and in focus anymore.
...
`Ìmportant` Do not move in X and Y. If the structure is not centered in Y like in the image below this is due to a small mechanical tilt between X- and A-Axis, which both run parallel to each other.   
