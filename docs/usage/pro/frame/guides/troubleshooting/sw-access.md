...
1. If the network service is deployed as a Docker container, [check the status of its Docker container(s)](#how-to-check-docker-container-state).

   Otherwise, if the network service is deployed as a systemd service, [check the status of its systemd service(s)](#how-to-check-systemd-service-state).

3. If the network service is supposed to be accessed via port 80, 443, or 8000, check whether other network services (e.g. ImSwitch, the openUC2 offline documentation, and the system file manager) are available over those ports by [opening their browser apps](../sw-access/README.md#browser-apps).
...
1. [Open the browser app](../sw-access/README.md#browser-apps) for Dozzle.
